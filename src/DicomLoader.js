 ...
 */
function DicomLoader() {

}

DicomLoader.loaded = false;

DicomLoader.enabled = false;
 ...
 */
DicomLoader.loadAndViewImage = function(imageId, addControls, cb) {
    var element = $('#dicomImage').get(0);
    if (!DicomLoader.enabled) {
        cornerstone.enable(element);
        DicomLoader.enabled = true;
    }
    ...
    cornerstone.loadImage(imageId).then(function(image) {
        ...
        if(DicomLoader.loaded === false && addControls) {
        ...
        DicomLoader.loadTexture(element, metadata);
        // DicomLoader.printData(image);
        cb();
    }, function(err) {
        console.error(err);
    });
...
DicomLoader.loadTexture = function(element, metadata) {
    if (!imageCanvas || !imageCtx) {
        imageCanvas = element.querySelector('canvas');
        imageCtx = imageCanvas.getContext('2d');
    }
    app.loadTexture(imageCanvas.toDataURL(), imageCtx.getImageData(0,0,imageCanvas.width, imageCanvas.height), metadata);
};
 ...
 */
DicomLoader.printData = function(image) {
